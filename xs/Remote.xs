...
ls(self)
	...
	CODE:
		...
		for (i = 0; i < count; ++i) {
			...
			if (local) {
				hv_stores(entry, "lid", git_oid_to_sv(&refs[i]->loid));
			}

			ref_name = refs[i]->name;
			len = strlen(ref_name) - (strstr(ref_name, peel) == NULL ? 0 : strlen(peel));

			hv_store(r, refs[i]->name, len, newRV_noinc((SV *) entry), 0);
		}
