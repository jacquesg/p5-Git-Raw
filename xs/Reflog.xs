...
delete(self)
	...
	CODE:
		ref = GIT_SV_TO_PTR(Reference,
			xs_object_magic_get_struct(aTHX_ SvRV(self)));

		rc = git_reflog_delete(git_reference_owner(ref),
			git_reference_name(ref));
		git_check_error(rc);
...
append(self, message, ...)
	...
	CODE:
		...
		rc = git_reference_name_to_id(&id, ref_owner,
			git_reference_name(ref));
		git_check_error(rc);
...
entries(self)
	...
	PPCODE:
		...
		for (i = 0; i < entry_count; ++i) {
			SV *committer;
			const git_reflog_entry *e = git_reflog_entry_byindex(
				reflog, i);
			...
			GIT_NEW_OBJ(committer, "Git::Raw::Signature",
				sig, SvRV(self));
